...
class KDNode:
    def __init__(self, K, split=0, parent:None|KDNode=None):
...
class KDTree:
    ...
    def load(self, idx, parent:None|KDNode):
        size = len(idx)
        dim = parent.dim if parent else 0
        if size == 0:
            return None
        elif size == 1:
            return KDNode(self.K, idx[0], parent)
        else:
            idx = idx[numpy.argsort(self.data[idx, dim])]
            node = KDNode(self.K, idx[size//2], parent)
            node.left = self.load(idx[:size//2], node)
            node.right = self.load(idx[size//2+1:], node)
            return node

    def locate(self, v, node:KDNode):
        if v[node.dim] <= self.data[node.split, node.dim]:
            return node if not node.left else self.locate(v, node.left)
        else:
            return node if not node.right else self.locate(v, node.right)
        
    def search(self, v, node:KDNode, visited:list):
        i = node.split            
        d = (v - self.data[node.split]).L
        visited[i] = True
        for node in [node.left, node.right, node.parent]:
            if node and not visited[node.split]:
                td, ti = self.search(v, node, visited)
                if td < d:
                    d, i = td, ti
        return d, i
    ...
    def query(self, values):
        ...
        for v in values:
            loc = self.locate(v, self.tree)
            visited = [False] * len(self.data)
            d, i = self.search(v, loc, visited)
            ds.append(d)
            ids.append(i)
